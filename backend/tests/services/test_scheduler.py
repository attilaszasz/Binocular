...
@pytest.mark.asyncio
async def test_scheduler_lifecycle(temp_db) -> None:
...
@pytest.mark.asyncio
async def test_reschedule_module(temp_db) -> None:
    conn, settings = temp_db
    scrape_client = MagicMock()
    ...
    cursor = await conn.execute("SELECT interval_hours FROM schedules WHERE module_id = 1")
...
@pytest.mark.asyncio
async def test_remove_job_on_status_change(temp_db) -> None:
    conn, settings = temp_db
    scrape_client = MagicMock()
...
async def test_run_module_check_triggers_checks(mock_check_device, temp_db) -> None:
    conn, settings = temp_db
    scrape_client = MagicMock()
    ...
    cursor = await conn.execute("SELECT last_run, next_run FROM schedules WHERE module_id = 1")
    row = await cursor.fetchone()
