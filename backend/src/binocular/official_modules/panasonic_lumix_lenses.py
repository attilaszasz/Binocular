...
def check_firmware(url: str, model: str, http_client: Any) -> dict[str, Any]:
    ...
    if not entries:
        raise ValueError(
            "firmware_index_not_found: Panasonic Lumix Lenses firmware table was"
            f" not found in page content at {source_url}"
        )

    entry = find_firmware_entry(entries, model)
    if entry is None:
        raise ValueError(
            f"product_not_found: Panasonic Lumix Lenses product was not found: {model}"
        )
    ...
    if not entry.firmware_download_url:
        raise ValueError(
            "download_url_not_found: Panasonic download URL is not available"
            f" for {entry.model}"
        )
    ...
    return {
        ...
        "product_name": f"Panasonic Lumix Lens {entry.model}",
