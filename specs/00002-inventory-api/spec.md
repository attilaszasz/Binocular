...
- **FR-007**: All resource representations MUST include the resource identifier and timestamps (`created_at`, `updated_at`, and `last_checked_at` when present) in RFC 3339 UTC format (`YYYY-MM-DDTHH:MM:SS.sssZ`) with millisecond precision.
- **FR-008**: The system MUST validate all input at the API boundary: names must be non-empty and no longer than 200 characters after trimming whitespace; firmware source URLs must be syntactically valid absolute `http` or `https` URLs and no longer than 2048 characters; numeric frequency values must be positive.
...
- **FR-008b**: All user-provided string fields (`name`, `firmware_source_url`, `current_version`, `notes`) MUST be canonicalized by trimming leading/trailing whitespace before validation and persistence.
- **FR-008c**: Unknown request-body fields MUST be rejected (not ignored) with `VALIDATION_ERROR`.
...
- **FR-012**: The system MUST support filtering the device list by device type and by update status (`update_available`, `up_to_date`, `never_checked`). Invalid filter values MUST be rejected with `VALIDATION_ERROR`.
- **FR-013**: The system MUST support sorting the device list by `name` and by `last_checked_at`. Allowed sort values are `name`, `-name`, `last_checked_at`, and `-last_checked_at`. The default sort order, when not specified by the caller, MUST be `name` ascending (alphabetical). Invalid sort values MUST be rejected with `VALIDATION_ERROR`.
- **FR-013b**: Sorting MUST be deterministic: ties on the primary sort field MUST use `id` ascending as a secondary tie-breaker. For `last_checked_at` sorting, `null` values (never checked) MUST always appear last.
...
- **FR-020**: The API MUST support request correlation via `X-Correlation-ID`: if provided by the caller it MUST be propagated unchanged; if absent it MUST be generated server-side. The effective correlation ID MUST be returned on both success and error responses.
- **FR-021**: The system MUST emit structured diagnostic logs for each API request containing at least: HTTP method, route template, status code, request duration (milliseconds), correlation ID, and error code (when applicable).
- **FR-022**: State-changing actions (create/update/delete/confirm/bulk-confirm) MUST emit timestamped audit events with action type, target resource identifier(s), outcome (`success`/`error`), and correlation ID.
