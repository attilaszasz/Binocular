...
- What happens when optional manifest constants (e.g., `MODULE_AUTHOR`, `MODULE_DESCRIPTION`) are present in a module file? They are NOT validated at load time — only `MODULE_VERSION` and `SUPPORTED_DEVICE_TYPE` are mandatory and validated. Optional constants are informational metadata stored in the registry if the system reads them, but their absence or invalid types do not affect module activation.
- What happens when a module file cannot be read due to filesystem permission errors? The import fails with a `PermissionError`. The module is registered as inactive with the permission error details, and a warning is logged. Other modules continue loading normally.
- What happens when a previously registered module's file is deleted from disk? On the next reload cycle (or restart), the module is deactivated in the registry (marked inactive with an error message indicating the file is missing). It is not removed from the registry — the historical record is preserved.
- What happens when the user deletes the mock module from `/app/modules/` but other module files remain? The seeding logic (FR-013a) only runs when the modules directory is completely empty. Selectively deleted built-in modules are NOT automatically re-seeded. The user must manually copy the file back from `/app/_modules/` or empty the directory to trigger a full re-seed.

## Requirements

### Functional Requirements

- **FR-001**: System MUST define a standard function signature that all extension modules implement: `check_firmware(url, model, http_client)` — accepting a firmware source URL (`url: str`), a device model identifier (`model: str`), and a host-provided HTTP client (`http_client: httpx.Client`) as inputs and returning a `dict` containing at minimum the latest firmware version string.
...
- **FR-003**: System MUST discover and load module files from a designated modules directory using safe dynamic import mechanisms — never `exec()` or `eval()`. Files whose names start with `_` (underscore) or are named `__init__.py` MUST be excluded from discovery.
- **FR-004**: System MUST validate each module at load time by verifying: (a) the required `check_firmware` function exists, (b) the function is callable, (c) the function's parameter signature matches the contract (exactly three parameters: `url`, `model`, `http_client` — validated via `inspect.signature()`), (d) mandatory manifest constants (`MODULE_VERSION`, `SUPPORTED_DEVICE_TYPE`) are present, and (e) manifest constants are strings.
...
- Q: How should error descriptions be formatted to let users distinguish "version not found" from "module crash"? → A: Use a consistent prefix pattern. Validation failures (including version-not-found): `"Validation failed: <detail>"` (e.g., `"Validation failed: latest_version is required — page was reachable but no version string could be extracted"`). Module exceptions: `"Module error: <ExceptionType>: <message>"`. Timeouts: `"Timeout: execution exceeded <N>s limit"`. This format is guidance for implementers, not a rigid schema.
